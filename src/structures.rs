                        ...
                        Ok(process_value) => {
                            ...
                            for topo in value.topos {
                                ...
                                match _create_topology {
                                    Ok(topology_value) => {
                                        let _add_topology = julia::call(&mut frame, &["Predicer", "add_topology"], &[process_value, topology_value]);
                                        match _add_topology {
                            ...
                            let _add_to_processes = julia::call(&mut frame, &["Predicer", "add_to_processes"], &[process_value]).into_jlrs_result();

                            let _topos_test = julia::call(&mut frame, &["Predicer", "topo_test"], &[process_value]).into_jlrs_result();
