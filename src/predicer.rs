...
pub fn _ordered_dict(data: Vec<(String, f64)>) {
    ...
    let _x = julia.scope(|mut frame| {
        ...
        let _result2 = julia_interface::to_ordered_dict(frame.as_extended_target(), &data).unwrap();

        let _result3 = julia_interface::call(&mut frame, &["Predicer", "print_ordered_dict"], &[_result2]).into_jlrs_result();
    ...
    unsafe {
        julia.scope(|mut frame| {
            ...
            for (key, value) in &processes {
                ...
                let process = julia_interface::call(&mut frame, &["Predicer", "create_process"], &[p_d1, p_d2]).into_jlrs_result();
                ...
                match process {
                    Ok(process_value) => {
                        ...
                        for topo in value.topos {
                            ...
                            let _create_topology = julia_interface::call(&mut frame, &["Predicer", "create_topology"], &[t_d1, t_d2, t_d3, t_d4, t_d5, t_d6]);
            ...
            let _processes = julia_interface::call(&mut frame, &["Predicer", "return_processes"], &[]).into_jlrs_result();
            ...
            for (key, value) in &nodes {
                ...
                let _node = julia_interface::call(&mut frame, &["Predicer", "create_node"], &[n_d1, n_d2, n_d3]).into_jlrs_result();

                match _node {
                    Ok(node_value) => {
                        let _add_to_nodes_result = julia_interface::call(&mut frame, &["Predicer", "add_to_nodes"], &[node_value]);
            ...
            let n_args = [];
            let _nodes = julia_interface::call(&mut frame, &["Predicer", "return_nodes"], &n_args).into_jlrs_result();
            ...
            for (key, value) in &node_diffusion {
                ...
                let _node_diffusion_tuple = julia_interface::call(&mut frame, &["Predicer", "create_node_diffusion_tuple"], &[nd_d1, nd_d2, nd_d3]).into_jlrs_result();
            ...
            let nd_args = [];
            let _node_diffusion_tuples = julia_interface::call(&mut frame, &["Predicer", "return_node_diffusion_tuples"], &nd_args).into_jlrs_result();
            ...
            for (key, value) in &node_delay {
                ...
                let _node_delay_tuple = julia_interface::call(&mut frame, &["Predicer", "create_node_delay_tuple"], &[nde_d1, nde_d2, nde_d3, nde_d4, nde_d5]).into_jlrs_result();
            ...
            let nde_args = [];
            let _node_delay_tuples = julia_interface::call(&mut frame, &["Predicer", "return_node_delay_tuples"], &nde_args).into_jlrs_result();
            ...
            let args1 = [];
            let node_histories = julia_interface::call(&mut frame, &["Predicer", "return_node_histories"], &args1).into_jlrs_result();
            ...
            for (key, value) in &markets {
                ...
                let _market = julia_interface::call(&mut frame, &["Predicer", "create_market"], &m_args).into_jlrs_result();

                match _market {
                    Ok(market_value) => {
                        let _add_to_markets_result = julia_interface::call(&mut frame, &["Predicer", "add_to_markets"], &[market_value]);
            ...
            let m_args = [];
            let _markets = julia_interface::call(&mut frame, &["Predicer", "return_markets"], &m_args).into_jlrs_result();
            ...
            for (key, value) in &groups {
                ...
                let _group = julia_interface::call(&mut frame, &["Predicer", "create_group"], &g_args).into_jlrs_result();

                match _group {
                    Ok(group_value) => {
                        let _add_to_groups_result = julia_interface::call(&mut frame, &["Predicer", "add_to_groups"], &[group_value]);
            ...
            let g_args = [];
            let _groups = julia_interface::call(&mut frame, &["Predicer", "return_groups"], &g_args).into_jlrs_result();
            ...
            // temporary data.
                ...
                let _genconstraint = julia_interface::call(&mut frame, &["Predicer", "create_genconstraint"], &gc_args).into_jlrs_result();

                match _genconstraint {
                    Ok(gc_value) => {
                        let _add_to_genconstraints_result = julia_interface::call(&mut frame, &["Predicer", "add_to_genconstraints"], &[gc_value]);
            ...
            let gc_args = [];
            let _genconstraints = julia_interface::call(&mut frame, &["Predicer", "return_genconstraints"], &gc_args).into_jlrs_result();
            ...
            let j_scenarios = julia_interface::to_ordered_dict(frame.as_extended_target(), &r_scenarios).unwrap();
            ...
            let j_reserve_type = julia_interface::to_ordered_dict(frame.as_extended_target(), &r_reserve_type).unwrap();
            ...
            let j_risk = julia_interface::to_ordered_dict(frame.as_extended_target(), &r_risk).unwrap();

            //inflow_blocks

            let args1 = [];
            let inflowblocks = julia_interface::call(&mut frame, &["Predicer", "return_inflowblocks"], &args1).into_jlrs_result();
...
pub fn _test(da1: i64, da2: i64, da3: i64, da4: i64, data: Vec<(String, f64)>) {
    ...
    let _x = julia.scope(|mut frame| {
        ...
        //let module = "Predicer";
        let _result = julia_interface::call(&mut frame, &["Predicer", "print_message"], &[d1, d2, d3, d4]).into_jlrs_result();
